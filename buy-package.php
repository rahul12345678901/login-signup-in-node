...
<?php
    if(isset($_GET['del'])){
        $del_id = $_GET['del']; 
       $del_check_query = "SELECT * FROM booking_query WHERE id = $del_id";
        $del_check_run = mysqli_query($conn, $del_check_query);
        if(mysqli_num_rows($del_check_run) > 0){
             $del_query = "DELETE FROM `booking_query` WHERE `booking_query`.`id` = $del_id";
       if(mysqli_query( $conn, $del_query)){
           ?><script>alert('booking deleted')</script><?php
			 }else{
				?><script>alert('booking not deleted')</script><?php
       }
        }
        else{
	...
	if(isset($_POST['checkboxes'])){
		foreach($_POST['checkboxes'] as $row_id){
		 ...
		 if($bulk_option == 'pending' ){
			 ...
			 $status = "UPDATE `booking_query` SET `status` = '$bulk_option' WHERE `booking_query`.`id` = $row_id;";
			...
			<div class="col-md-9 ">
				...
				<div class="row">
					...
					<?php 
                        if(isset($_GET['reply'])){
                            $reply_id = $_GET['reply'];
                            $reply_check = "SELECT * FROM booking_query WHERE id=$reply_id";
					...
					<?php 
							...
							$query = "SELECT * FROM booking_query ORDER BY id DESC";
						...
						<table class="table table-hover table-bordered table-striped" id="example"  cellspacing="0" width="100%">
							...
							<tbody>
                                ...
                                while($row = mysqli_fetch_array($run))
                                {
                                    ...
                                    $row_id = $row['id'];
                                    $row_package_name = $row['package_name'];
                                    $row_name = $row['name'];
								...
								<tr>
									...
									<td><?php echo $a; ?></td>
									<td><?php echo $row_package_name; ?></td>
									...
									<td><a href="buy-package.php?reply=<?php echo  $row_id;?>"><i class="fa fa-reply"></i></a></td>
									<td><a href="buy-package.php?del=<?php echo $row_id;?>"><i class="fa fa-times"></i></a></td>
								</tr>
							<?php $a++; } ?>
